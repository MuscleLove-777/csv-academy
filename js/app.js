/* ============================================
   Generic Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('csv-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('csv-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('csv-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('csv-progress', JSON.stringify(this.progress));
        localStorage.setItem('csv-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>CSV Academy へようこそ</h2>
                    <p>コンピュータシステムバリデーション（CSV）の基礎から実践までを体系的に学ぶアカデミーです。GxP環境におけるシステムの信頼性確保と規制対応力を身につけましょう。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('csv-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('csv-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('csv-progress');
            localStorage.removeItem('csv-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
